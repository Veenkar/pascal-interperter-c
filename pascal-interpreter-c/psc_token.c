...
int Psc_Token_String(const Psc_Token_T *self_, char *buf_, size_t bufsize)
{
    ...
    else /** psc_token not in psc_token enum - str repr psc_token_enum_name not avaiable */
    {
        return -2;
    }
    ...
    switch (self_->type)
    {
        case PSC_TOKEN_INT:
        {
            ...
            return snprintf(buf_, bufsize, "Psc_Token(%s, %d)", psc_token_enum_name, int_val);
        }
        ...
        case PSC_TOKEN_DIV:
        {
            ...
            return snprintf(buf_, bufsize, "Psc_Token(%s, %c)", psc_token_enum_name, char_val);
        }
